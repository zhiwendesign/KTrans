...
- **网页划词快捷键**：网页处于焦点时触发，默认是 `Alt + Q`，可在 KTrans 设置页直接录制或清除。
- **浏览器全局快捷键**：由浏览器管理，默认值如下，可通过设置页的“管理浏览器全局快捷键”打开管理页面修改。
...
设置网页划词快捷键时必须搭配 `Ctrl`、`Alt` 或 `Command`，以免影响网页正常输入；按 `Delete` 或 `Backspace` 可以清除。
